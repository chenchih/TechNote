...
> syntax: `uvicorn app [options]`

#### Method1- manual run the server by command
...
#### Method2- write server into `__main__():`
...
In above start server, I use some flag or option, I want to explain each flag in case you want to know. So you also use this command `uvicorn --help` to see all options. I will not mention all of them, only pick I think it's useful or commonly use. You can reference [ucicorn offical site](https://www.uvicorn.org/)
...
- `filename:app`: Replace filename with the actual file name, default use main as default app, which mean not specify filename will use main.py.
...
### Run the API by send Request or accessing endpoint
...
### Missing Field of data raise Error

Let me show is your field not match DataModel, then what will happen? It will raise error, let me show you an example.
...
#### Solution for fixing missing field
...
![missing_None_responsemodel](<[img/responseModel_example.png](https://raw.githubusercontent.com/chenchih/TechNote/main/PythonNote/Project_Youtube/Create_FastApi/img/response_model.PNG)>)
