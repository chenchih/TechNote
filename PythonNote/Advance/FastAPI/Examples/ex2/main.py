...
#output please use the userout's model
@app.get(path: '/users/{username}', response_model=UserOut)
async def get_user(username: str):
    return users.get(username, {})
#response_model_include={"id"} #will only output id
#response_model_exclude={"id", "username"} # not display id and username
#response_model_exclude_unset=True #comapre value and return value, no match not display output
